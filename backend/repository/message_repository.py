...
def get_messages(conv_id: str) -> List[MessageModel]:
    cursor =  mongodb.message.find({"conv_id": conv_id}).sort("ts", 1)

    return [MessageModel(**msg) for msg in list(cursor)] if cursor is not None else []

def get_latest_message(conv_id: str, limit = 10) -> List[MessageModel]:
    cursor = mongodb.message.find({"conv_id": conv_id}).sort("ts", 1).limit(limit)
